...
abstract contract SphereXProtectedRegisteredBase {
  ...
  //避免冲突：通过使用一个复杂的字符串（'eip1967.spherex.spherex_engine'）
  //作为输入，生成的存储槽很难与合约中的其他存储变量发生冲突。
   ...
   */
    /**
   * @dev WildcatArchController 部署的地址。
   *      arch controller（架构控制器）能够设置 SphereX 引擎地址。
   *      继承此合约的合约必须在其构造函数中分配这个地址。
   */
   ...
   *    * @dev  更改 SphereX 引擎的地址。
   *
   *       这也用于启用 SphereX 保护，当引擎地址为 0 时，保护功能会被禁用。
   *
   * 注意：新的引擎地址不会像在 `SphereXProtectedBase` 中那样被验证，
   *       这是因为操作者是 arch controller（架构控制器），
   *       它会在更新引擎地址之前对其进行验证。
   * 1.函数的主要目的是更改 SphereX 引擎的地址。
   * 2.这个函数还用于启用或禁用 SphereX 保护功能：
   *   当设置一个非零地址时，保护功能被启用。
   *   当设置为零地址时，保护功能被禁用。
   */
  function changeSphereXEngine(address newSphereXEngine) external spherexOnlyOperator {
    ...
    // 触发 ChangedSpherexEngineAddress 事件
   ...
   */
  /**
 * @dev 这是 `_getStorageSlotsAndPreparePostCalldata` 的包装函数，
 *      它返回一个指向 `locals` 的 `uint256` 指针，而不是结构体本身。
 *
 *      为结构体声明返回参数时，总是会为结构体的每个字段分配内存并将其置零。
 *      如果这个参数总是被重新赋值，那么用于此的 gas 和内存就会被浪费。
 *
 *      使用 `uint256` 指针而不是结构体声明可以避免这种浪费，
 *      同时在功能上保持完全相同。
 */
