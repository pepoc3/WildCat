...
library LibERC20 {
  ...
  function safeTransfer(address token, address to, uint256 amount) internal {
    /// @solidity memory-safe-assembly // 使用内存安全的汇编代码
    assembly {
      mstore(0x14, to) // Store the `to` argument. // 将 `to` 参数存储在内存中
      mstore(0x34, amount) // Store the `amount` argument. // 将 `amount` 参数存储在内存中
      mstore(0x00, 0xa9059cbb000000000000000000000000) // `transfer(address,uint256)`. // 将 `transfer(address,uint256)` 函数选择器存储在内存中
      // Perform the transfer, reverting upon failure. // 执行转账，如果失败则抛出错误
      if iszero(
        and(
          // The arguments of `and` are evaluated from right to left. // 从右到左评估 `and` 的参数
          or(eq(mload(0x00), 1), iszero(returndatasize())), // Returned 1 or nothing. // 返回 1 或没有返回值
          call(gas(), token, 0, 0x10, 0x44, 0x00, 0x20) // 调用 `token` 合约的 `transfer(address,uint256)` 函数
        )
      ) {
        mstore(0x00, 0x90b8ec18) // `TransferFailed()`. // 将 `TransferFailed()` 错误选择器存储在内存中
        revert(0x1c, 0x04) // 抛出错误
      }
      mstore(0x34, 0) // Restore the part of the free memory pointer that was overwritten. // 恢复被覆盖的内存指针部分
    }
  }
